...
class BusinessInsightAnalyzer:
    ...
    def analyze_reviews_by_aspects(
        ...
        if len(processed_texts) != len(df_with_sentiment):
            raise ValueError(
                f"Количество обработанных текстов ({len(processed_texts)}) "
                f"не совпадает с количеством строк DataFrame ({len(df_with_sentiment)})"
            )
        
        # ─────────────────────────────────────────────────────────
        # Создаем тексты по тональности напрямую из processed_texts
        # ─────────────────────────────────────────────────────────
        sentiments_col = df_with_sentiment["sentiment"]
        
        texts_by_sentiment = {
            "positive": [processed_texts[i] for i, s in enumerate(sentiments_col) if s == "positive"],
            "negative": [processed_texts[i] for i, s in enumerate(sentiments_col) if s == "negative"],
            "neutral":  [processed_texts[i] for i, s in enumerate(sentiments_col) if s == "neutral"],
        }
